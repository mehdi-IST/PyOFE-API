...
def show_fit_result():
    ...
    # Search for the most recent JSON file in subdirectories
    for root, dirs, files in os.walk(download_folder):
        for file in files:
...
# Set the window size
root.geometry("1100x750+0+0")  # "+0+0" positions it at top-left corner
...
function_entry = tk.Text(function_entry_frame, height=12, width=128, wrap=tk.WORD, font=("Arial", 10))
...
result_text = tk.Text(result_frame, height=12, wrap=tk.NONE, font=("Arial", 10))
...
gnuplot_combined_frame.columnconfigure(0, weight=1)  # 1/4 width
gnuplot_combined_frame.columnconfigure(1, weight=3)  # 3/4 width
...
# Same height as gnuplot_input (8), narrower width (since it's 1/4 of screen)
process_text = tk.Text(process_frame, height=8, width=30, font=("Courier", 10))
...
# Wide text box, same height as process_text
